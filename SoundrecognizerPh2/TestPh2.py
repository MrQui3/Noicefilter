...
interpreter = tf.lite.Interpreter(model_path='model_recognizerPh2.tflite')
...
def run_inference(input_data):
    ...
    print(np.argmax(output_data[0][0], axis=1))
    return np.argmax(output_data[0][0], axis=1)

Mus = '../Data/testdata/musicdata'
Not_Mus = '../Data/testdata/rauschdata'
Mus_with_noise = '../Data/testdata/ueberlagerungvonrauschen'
print(Mus)
...
training_data_mus_with_noise = []
training_data_mus_with_noise = settrainingsdata(Mus_with_noise)
...
print('---------Recognition of music with noise-----------')
music_with_noise_it_is = 0
for single_data in training_data_mus_with_noise:
    prediction = run_inference(single_data)
    answer = prediction
    #Label 2 = Music with noise
    if answer == 2:
        music_with_noise_it_is += 1
        print('To Music with noise the prediction is: music with noise')
    else:
        print('To Music with noise the prediction is: not music with noise')
    predictions.append([2, answer])

accuracy = music_with_noise_it_is / len(training_data_mus_with_noise)
print('The accuracy of the music with noise recognition is: ', accuracy)
